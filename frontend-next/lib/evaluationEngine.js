/**
 * Claude API 기반 평가 엔진 — ES 모듈 버전 (Vercel API Routes용)
 * backend/services/evaluationEngine.js 와 동일한 로직
 */
...
- 미달: ${rubric.levelThresholds.보통}점 미만`;
  ...
  validateScores(result, practiceType) {
    ...
    for (const [key] of Object.entries(rubric.items)) {
      const scoreData = result.item_scores[key];
      if (scoreData) {
        const score = Math.max(1, Math.min(4, Math.round(Number(scoreData.score) || 1)));
        validatedScores[key] = { score, evidence: scoreData.evidence || '', feedback: scoreData.feedback || '' };
        totalRaw += score;
        count++;
      } else {
...
【FACT 프레임워크】
- F (Feasibility, 실현력): 아이디어를 실제로 구현하는 능력
- A (AI literacy, AI 리터러시): AI를 비판적으로 활용하는 능력
- C (Critical thinking, 비판적 사고): 정보를 분석·평가·종합하는 능력
...
\`\`\`json
{
  ...
  "total_score": 전체평균,
  "overall_narrative": "한 학기 성장 스토리 (300자 내외)",
...
한국어로 작성하며, 실제 산출물 내용을 인용합니다.`;
    ...
    artifacts.forEach((artifact, i) => {
      ...
      artifactsSummary += `\n【산출물 ${i + 1}: ${type} (${artifact.date || '미표기'})】\n${text}\n`;
    });
...
【예시】
"인공지능 활용 프로젝트에서 '교실 내 온습도 실시간 모니터링 시스템'을 주제로 선정함. 문제 정의 단계에서 수업 시간 데이터를 수집하여 '4교시 이후 CO2 농도 1000ppm 초과'라는 구체적 문제를 도출함."
